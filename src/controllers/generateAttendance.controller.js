...
// ---------------------------------------------------------------------------
// Generate Attendance (port of the WinForms frmGenerateAttendance)
//
//   Pick Pay Type -> Pay Period (auto-fills From/To, constrained to the period),
//   then loop DAY BY DAY from From to To calling sp_Generate_Attendance_New_Unit1
//   (@FromDate, @ToDate = the same day, @PayTypeCode, @CompanyCode; 600s timeout),
//   exactly like the desktop's btnGenerate loop + lblDays counter.
//
//   The day loop can run for minutes — longer than the gateway's 60s forwarder
//   timeout — so /generate runs as a BACKGROUND JOB and the UI polls /progress
//   (which also drives the live day counter "Day X / N", like lblDays).
//   Job state is in-memory (single on-prem core process).
//
//   Endpoints
...
// GET /generate-attendance/options  -> pay types
export const getOptions = async (req, res) => {
  try {
    ...
    console.error("DB Error (GenerateAttendance.getOptions):", err);
    return sendError(res, err);
  }
};

// GET /generate-attendance/pay-periods/:payTypeCode  -> open pay periods (+From/To)
export const getPayPeriods = async (req, res) => {
  try {
    ...
    console.error("DB Error (GenerateAttendance.getPayPeriods):", err);
    return sendError(res, err);
  }
};

// The background worker — loop day-by-day calling sp_Generate_Attendance_New_Unit1.
  ...
  try {
    ...
    for (const d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      try {
        ...
        // desktop catches per-day and continues the loop
    ...
    console.error("DB Error (GenerateAttendance.runGenerate):", err);
  }
}

// POST /generate-attendance/generate  -> start a background generate
export const startGenerate = async (req, res) => {
  try {
    ...
    console.error("DB Error (GenerateAttendance.startGenerate):", err);
    return sendError(res, err);
  }
};

// GET /generate-attendance/progress/:runId
