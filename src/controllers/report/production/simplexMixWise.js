// Simplex Production Details (Mix Wise) report.
// Mirrors rptSimplexMixWiseDetails_report.rdlc — one row per Mixing with Opening
// + per-shift production + Total + Upto Date, plus a grand-total row.
//
// SP: sp_Prodn_Simplex_ShiftMixWise (CompanyCode, FromDate, ToDate[, ShiftCode])
...
const TITLE = 'SIMPLEX PRODUCTION DETAILS (MIX WISE)';
const FILE_NAME = 'SimplexProduction_MixWise';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'SIMPLEX MIX WISE - SUMMARY',
    groupHeader: 'Mixing',
    groupSummaries,
...
export const simplexMixWiseReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_Simplex_ShiftMixWise', fileName: FILE_NAME, buildDocDefinition });
