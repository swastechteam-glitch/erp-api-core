...
// Mirrors rptDrawingEmployeePerformance.rdlc — one row per employee with
// Production (Kg) / Efficiency (%) / Utilisation (%) / Index, plus a footer
// with the employee count + total production + average eff / util / index.
//
// SP: sp_Prodn_Drawing_EmployeePerformance (CompanyCode, FromDate, ToDate)
...
const TITLE = 'BREAKER DRAWING EMPLOYEE PERFORMANCES REPORT';
const FILE_NAME = 'DrawingProduction_EmployeePerformance';
...
export const drawingEmployeePerformanceReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_Drawing_EmployeePerformance', fileName: FILE_NAME, buildDocDefinition });
