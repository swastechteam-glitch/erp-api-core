// Comber Production Details (Mix Wise) report.
// Mirrors rptComberMixWiseDetails_report.rdlc — one row per Mixing with Opening
// + per-shift production + Total + Upto Date, plus a grand-total row.
//
// SP: sp_Prodn_Comber_ShiftMixWise (CompanyCode, FromDate, ToDate[, ShiftCode])
...
const TITLE = 'COMBER PRODUCTION DETAILS (MIX WISE)';
const FILE_NAME = 'ComberProduction_MixWise';
...
export const comberMixWiseReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_Comber_ShiftMixWise', fileName: FILE_NAME, buildDocDefinition });
