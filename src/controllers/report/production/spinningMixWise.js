// Spinning Production Details (Mix Wise) report.
// Mirrors rptSpinningMixWiseDetails_report.rdlc — one row per Mixing with Opening
// + per-shift production + Total + Upto Date, plus a grand-total row.
//
// SP: sp_Prodn_Spinning_ShiftMixWise (CompanyCode, FromDate, ToDate)
...
const TITLE = 'SPINNING PRODUCTION DETAILS (MIX WISE)';
const FILE_NAME = 'SpinningProduction_MixWise';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'SPINNING MIX WISE - SUMMARY',
    groupHeader: 'Mixing',
    groupSummaries,
...
export const spinningMixWiseReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_Spinning_ShiftMixWise', fileName: FILE_NAME, buildDocDefinition });
