...
// Mirrors rptComberProductionShiftWise_Details.rdlc — like Shift Wise but with
...
const FILE_NAME = 'ComberProduction_ShiftWiseDetails';
  ...
  for (const shiftRows of groups.values()) {
    ...
    tables.push({
      ...
      table: {
        widths: ['auto', '*', '*', '*'],
        body: [[
          { text: `Date : ${ddmmyyyy(head.CBRProdnDate)}`, bold: true, fontSize: 8, fillColor: colors.subFill, color: colors.subText },
...
export const comberShiftWiseDetailsReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_ComberProdnDetails_GetAll', fileName: FILE_NAME, buildDocDefinition });
