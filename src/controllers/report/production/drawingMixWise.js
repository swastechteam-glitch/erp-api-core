// Breaker Drawing Production Details (Mix Wise) report.
// Mirrors rptDrawingMixWiseDetails_report.rdlc — one row per Mixing with
// Opening + per-shift production + Total + Upto Date, plus a grand-total row.
//
// SP: sp_Prodn_Drawing_ShiftMixWise (CompanyCode, FromDate, ToDate[, ShiftCode])
...
const TITLE = 'BREAKER DRAWING PRODUCTION DETAILS (MIX WISE)';
const FILE_NAME = 'DrawingProduction_MixWise';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'BREAKER DRAWING MIX WISE - SUMMARY',
    groupHeader: 'Mixing',
    groupSummaries,
...
export const drawingMixWiseReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_Drawing_ShiftMixWise', fileName: FILE_NAME, buildDocDefinition });
