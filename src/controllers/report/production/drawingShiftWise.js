// Breaker Drawing Production Shift Wise report.
// Mirrors rptDrawingProductionShiftWise_Arun.rdlc — detail rows grouped per
// shift, each shift block headed by Date / Shift / SIC / MON, per-shift totals.
//
// SP: sp_Prodn_DrawingProdnDetails_GetAll (CompanyCode, FromDate, ToDate)
...
// 10 columns: M/C No, Mixing, Working Mins, Speed, STD Prodn, ACT Prodn,
// Stop Time, Eff %, UT %, Index.
const WIDTHS = [38, '*', 56, 54, 56, 56, 54, 46, 46, 46];

const TITLE = 'BREAKER DRAWING PRODUCTION - SHIFT WISE';
const FILE_NAME = 'DrawingProduction_ShiftWise';
...
function buildShiftTable(shiftRows) {
  ...
  const idxAvg = (avg(sEff) + avg(sUtil)) / 2;
  body.push([
    ...
    { text: fmt(idxAvg, 2), alignment: 'right', ...gStyle },
  ]);
  ...
  for (const shiftRows of groups.values()) {
    ...
    tables.push({
      ...
      table: {
        widths: ['auto', '*', '*', '*'],
        body: [[
          { text: `Date : ${ddmmyyyy(head.DRWProdnDate)}`, bold: true, fontSize: 8, fillColor: colors.subFill, color: colors.subText },
...
export const drawingShiftWiseReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_DrawingProdnDetails_GetAll', fileName: FILE_NAME, buildDocDefinition });
