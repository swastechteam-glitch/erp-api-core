// Uni Lap Employee Performance report.
// Mirrors rptUnilapEmployeePerformance.rdlc — one row per employee with
// Production (Kg) / Efficiency (%) / Utilisation (%) / Index, plus a footer
// with the employee count + total production + average eff / util / index.
//
// SP: sp_Prodn_Unilap_EmployeePerformance (CompanyCode, FromDate, ToDate)
...
const WIDTHS = [36, 80, '*', 90, 90, 90, 80];

const TITLE = 'UNILAP EMPLOYEE PERFORMANCES REPORT';
const FILE_NAME = 'UnilapProduction_EmployeePerformance';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  body.push(['S.No', 'Employee ID', 'Employee Name', 'Production (Kg)', 'Efficiency (%)', 'Utilisation (%)', 'Index (%)'].map((h) => ({ text: h, ...headStyle })));

  let sProdn = 0, sEff = 0, sUtil = 0, sIdx = 0, n = 0;
  rows.forEach((r, i) => {
    ...
    sProdn += prodn; sEff += eff; sUtil += util; sIdx += idx; n++;
  ...
  body.push([
    ...
    { text: fmt(avg(sIdx), 2), alignment: 'right', ...gStyle },
  ]);
...
export const unilapEmployeePerformanceReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_Unilap_EmployeePerformance', fileName: FILE_NAME, buildDocDefinition });
