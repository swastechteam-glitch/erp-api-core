// Carding Production Details (Mix Wise) report.
// Mirrors rptCardingMixWiseDetails_report.rdlc — one row per Mixing with
// Opening + per-shift production + Total + Upto Date, plus a grand-total row.
//
// SP: sp_Prodn_Carding_ShiftMixWise (CompanyCode, FromDate, ToDate[, ShiftCode])
...
// 8 columns: S.No, Mixing, Opening, Shift 1, Shift 2, Shift 3, Total, Up to Date.
const WIDTHS = [40, '*', 80, 80, 80, 80, 90, 100];

const TITLE = 'CARDING PRODUCTION DETAILS (MIX WISE)';
const FILE_NAME = 'CardingProduction_MixWise';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'CARDING MIX WISE - SUMMARY',
    groupHeader: 'Mixing',
    groupSummaries,
...
export const cardingMixWiseReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_Carding_ShiftMixWise', fileName: FILE_NAME, buildDocDefinition });
