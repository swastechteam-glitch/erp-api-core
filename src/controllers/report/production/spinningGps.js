// Spinning GPS reports — Machine Wise and Count Wise.
// Mirrors rptSpinning_MachineWiseGPS.rdlc / rptSpinning_CountWiseGPS.rdlc —
// production + average actual GPS, grouped by machine or by count.
//
// SP: sp_Prodn_Spinning_GPS (CompanyCode, FromDate, ToDate)
...
export const spinningMachineWiseGpsReport = (req, res) =>
  runReport(req, res, {
    spName: 'sp_Prodn_Spinning_GPS',
    fileName: 'SpinningProduction_MachineWiseGPS',
    buildDocDefinition: (ctx) => buildGpsDoc(ctx, {
      title: 'MACHINE WISE SPINNING GPS',
      keyField: 'MachineCode', labelField: 'MachineName', groupHeader: 'Machine Name',
    }),
  });

export const spinningCountWiseGpsReport = (req, res) =>
  runReport(req, res, {
    spName: 'sp_Prodn_Spinning_GPS',
    fileName: 'SpinningProduction_CountWiseGPS',
    buildDocDefinition: (ctx) => buildGpsDoc(ctx, {
      title: 'COUNT WISE SPINNING GPS',
