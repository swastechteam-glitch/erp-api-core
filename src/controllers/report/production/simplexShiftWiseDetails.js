...
// Mirrors rptSimplexProductionShiftWise_Details.rdlc — like Shift Wise but with
...
const FILE_NAME = 'SimplexProduction_ShiftWiseDetails';
  ...
  for (const shiftRows of groups.values()) {
    ...
    tables.push({
      ...
      table: {
        widths: ['auto', '*', '*', '*'],
        body: [[
          { text: `Date : ${ddmmyyyy(head.SpxProdnDate)}`, bold: true, fontSize: 8, fillColor: colors.subFill, color: colors.subText },
...
export const simplexShiftWiseDetailsReport = (req, res) =>
  runReport(req, res, { spName: 'sp_Prodn_SimplexProdnDetails_GetAll', fileName: FILE_NAME, buildDocDefinition });
