...
// ---------------------------------------------------------------------------
// Attendance & OT Approval — Stage 1  (port of frmEmployeeAttendanceOffLineApproval).
//
//   Lists the pending manual attendance/OT entries (sp_ManualEntryApproval_Pendings
//   @CompanyCode). Tick one or more and Approve runs
//   sp_ManualEntryApproval_AddEdit @User, @Node, @Approval=1, @ManualCode for each
//   ticked row, then the list refreshes.
//
//   Company-scoped; user/node from the auth token.
//
//   Endpoints
//     GET   /pendings    sp_ManualEntryApproval_Pendings
//     POST  /approve     sp_ManualEntryApproval_AddEdit (selected, txn)
// ---------------------------------------------------------------------------

const APPROVAL_STAGE = 1; // Stage 1 (@Approval = 1)
...
export const getPendings = async (req, res) => {
  try {
    ...
    console.error("DB Error (AttenApproval.getPendings):", err);
    return sendError(res, err);
  }
};

// POST /atten-approval/approve  -> sp_ManualEntryApproval_AddEdit for selected rows (txn)
export const approve = async (req, res) => {
  ...
  try {
    ...
    for (const manualCode of manualCodes) {
      ...
      rq.input("Approval", sql.Int, APPROVAL_STAGE);
      rq.input("ManualCode", sql.Int, manualCode);
      await rq.execute("sp_ManualEntryApproval_AddEdit");
    }
    ...
    console.error("DB Error (AttenApproval.approve):", err);
    return sendError(res, err);
  }
};
