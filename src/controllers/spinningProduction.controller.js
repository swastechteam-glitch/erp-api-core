...
// ---------------------------------------------------------------------------
// Spinning Production Entry
//   (port of WinForms frmSpinningProduction_New / frmSpinningProductionDetails_New)
//   Header (No / Date / Branch / Shift / Supervisor / Monitor + Adash Cops) and
//   one row per spinning frame (loaded from the branch's frame settings). Master
//   values (Actual Count / TPI / Spindle / Speed / Spindle Constant / Std Effi /
//   40s Factor / Target GPS) come from vw_Prodn_Spinning_FrameSetting; the user
//   keys Employee / Hank / Waste / EB & Doff details; the server recomputes every
//   derived column from the VB unbound expressions:
//     Prodn.Conts  = round(1/2.2046/ActualCount * NoOfSpindle * (StdEffi/100), 2)
//     Speed        = round(Hank * Prodn.Conts, 2)
//     Std Prodn    = round(TargetGPS * NoOfSpindle / 1000, 2)
...
//     GMS/SPL      = WorkedSpl==0 ? 0 : round(ActProdn*1000/WorkedSpl, 2)
//     40s Value    = round(GMS/SPL * 40sFactor, 2)
//     UT           = round(100 - StopTime/WorkingMins*100, 2)
//     Effi         = (UT>0 && StdProdn>0) ? round(ActProdn/StdProdn*100*100/UT, 2) : 0
...
//   sp_Prodn_Spinning_FramSetting_Update per row (writes back the latest setting).
...
const computeRow = (d, workingMins, stopTime) => {
  ...
  const stdProdn = r2((targetGPS * noOfSpindle) / 1000);
  ...
  const gmsSpl = workedSpindle === 0 ? 0 : r2((actProdn * 1000) / workedSpindle);
  const conv40sValue = r2(gmsSpl * conv40sFactor);
  const ut = stopTime > 0 ? (workingMins > 0 ? r2(100 - (stopTime / workingMins) * 100) : 0) : 100;
  const effi = ut > 0 && stdProdn > 0 ? r2((actProdn / stdProdn) * 100 * 100 / ut) : 0;
...
// GET /spinning-production/options
export const getOptions = async (req, res) => {
  try {
    ...
    console.error("DB Error (getOptions spinning-production):", err);
    return sendError(res, err);
  }
};

// GET /spinning-production/machines?branchCode=
export const getMachines = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query(
        "Select * from vw_Prodn_Spinning_FrameSetting Where Status = 1 AND CompanyCode = " +
    ...
    console.error("DB Error (getMachines spinning-production):", err);
    return sendError(res, err);
  }
};

// GET /spinning-production/next-no
export const getNextNo = async (req, res) => {
  try {
    ...
    const no = await scalar(
      ...
      "sp_Prodn_SpinningProdnNo"
    );
    return sendSuccess(res, { no });
  } catch (err) {
    console.error("DB Error (getNextNo spinning-production):", err);
    return sendError(res, err);
  }
};

// GET /spinning-production/exists?date=&shiftCode=&branchCode=
export const checkExisting = async (req, res) => {
  try {
    ...
    const r = await pool
      ...
      .query(
        "SELECT COUNT(*) AS Cnt FROM vw_Prodn_SpinningProdn " +
    ...
    console.error("DB Error (checkExisting spinning-production):", err);
    return sendError(res, err);
  }
};

// GET /spinning-production/lists
export const getList = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query(`SELECT * FROM vw_Prodn_SpinningProdn WHERE CompanyCode = ${companyCode} AND FYCode = ${fyCode} ORDER BY SpgProdnNo DESC`);
    const data = result.recordset.map((item) => ({ ...item, id: item.SpgProdnCode }));
    return sendPaginated(res, data, req.query);
  } catch (err) {
    console.error("DB Error (getList spinning-production):", err);
    return sendError(res, err);
  }
};

// GET /spinning-production/list/:code?shiftCode=
export const getById = async (req, res) => {
  try {
    ...
    const detResult = await pool
      ...
      .input("ShiftCode", sql.Int, shiftCode)
      .execute("sp_Prodn_SpinningProdnDetails_GetAll");
    const rows = detResult.recordset || [];
    if (!rows.length) return sendError(res, "Spinning Production not found", 404);
    ...
    const stopResult = await pool
      .request()
      .query(`select * from vw_Prodn_SpinningStoppage where CompanyCode = ${companyCode} AND SpgProdnCode = ${code}`);
    ...
    console.error("DB Error (getById spinning-production):", err);
    return sendError(res, err);
  }
};
...
const saveOrUpdate = async (req, res, isEdit) => {
  ...
  try {
    ...
    const spgProdnCode = await scalar(hReq, "sp_Prodn_SpinningProdn_AddEdit");
    if (!spgProdnCode) throw new Error("Header save returned no SpgProdnCode");

    // Details: delete then per-row insert.
    await new sql.Request(tx).input("SpgProdnCode", sql.Int, spgProdnCode).execute("sp_Prodn_SpinningProdnDetails_Delete");
    ...
    for (const { d, c, stopTime, reason } of computed) {
      ...
      await dr.execute("sp_Prodn_SpinningProdnDetails_Insert");
    }

    // Stoppage: delete then insert.
    await new sql.Request(tx).input("SpgProdnCode", sql.Int, spgProdnCode).execute("sp_Prodn_SpinningStoppage_Delete");
    for (const s of stoppages) {
      ...
      await sr.execute("sp_Prodn_SpinningStoppage_Insert");
    }
    ...
    await new sql.Request(tx)
      ...
      .input("CompanyCode", sql.Int, companyCode)
      .execute("sp_Prodn_SpinningEmployee_Delete");
    for (const { d } of computed) {
      ...
      await er.execute("sp_Prodn_SpinningEmployee_Insert");
    }
    ...
    if (!isEdit) {
      for (const { d, c } of computed) {
        ...
        await ur.execute("sp_Prodn_Spinning_FramSetting_Update");
      }
    }
    ...
    console.error("DB Error (saveOrUpdate spinning-production):", err);
    return sendError(res, err);
  }
};

// POST /spinning-production/create
export const create = (req, res) => saveOrUpdate(req, res, false);
// PUT  /spinning-production/update/:code
export const update = (req, res) => saveOrUpdate(req, res, true);

// DELETE /spinning-production/delete/:code
export const remove = async (req, res) => {
  try {
    ...
    if (!code) return sendError(res, "Invalid SpgProdnCode", 400);
    const pool = await getPool(req.headers.subdbname);
    await pool.request().input("SpgProdnCode", sql.Int, code).execute("sp_Prodn_SpinningProdn_Delete");
    return sendSuccess(res, null, "The record is deleted");
  } catch (err) {
    if (err.message && (err.message.includes("REFERENCE") || err.message.includes("FK_"))) {
      return sendError(res, "You can not delete the Spinning!", 409);
    }
    console.error("DB Error (remove spinning-production):", err);
    return sendError(res, err);
  }
};
