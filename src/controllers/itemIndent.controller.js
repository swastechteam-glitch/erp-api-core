...
//   Same store proc family as Item Requisition, but for ISSUE: only in-stock
//   items, Qty must not exceed stock, and the auto-approval flags depend on the
//   item's rate vs the tbl_Setting thresholds. Indent number is "IND"+n.
//   - Options  : branches / cost heads / departments / employees / in-stock items
//   - Machines : tbl_Machine for a branch (+ DepartmentCode for filtering)
//   - Next no  : sp_ItemRequisition_ItemRequisitionNo (@RequisitionType='I') -> "IND"+n
//   - List     : sp_ItemRequisition_GetAll (@RequisitionType='I')
...
export const getOptions = async (req, res) => {
  try {
    ...
    const [branches, costHeads, departments, employees, items] = await Promise.all([
      ...
      // @Stock=1 -> only items that currently have stock (indent = issue).
      pool
        .request()
  ...
  try {
    ...
    for (const d of details) {
      if (toNum(d.Qty) > toNum(d.Stock))
        return sendError(res, "Please check the Issue Qty (exceeds stock)", 400);
    }
    ...
    // Auto-approval thresholds (issue value bands) from tbl_Setting.
    let appr1 = 0;
    let appr2 = 0;
    try {
      const s = await pool
    ...
    const strReqNo = isEdit ? b.strItemRequisitionNo || `IND${reqNo}` : `IND${reqNo}`;
    ...
    for (const d of details) {
      ...
      // Above either approval band -> needs approval (flag 0); else auto-pass (1).
      const needsApproval = rate >= appr1 || rate >= appr2;
      const appr = needsApproval ? 0 : 1;
      await new sql.Request(tx)
        .input("ItemRequisitionCode", sql.Int, itemRequisitionCode)
        ...
        .input("StockRate", sql.Decimal(18, 3), rate)
        .input("IssueApproval1", sql.Int, appr)
        .input("IssueApproval1_Auto", sql.Int, 0)
        .input("IssueApproval2", sql.Int, appr)
        .input("IssueApproval2_Auto", sql.Int, 0)
