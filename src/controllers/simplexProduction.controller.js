...
// ---------------------------------------------------------------------------
// Simplex Production Entry
//   (port of WinForms frmSimplexProduction_New / frmSimplexProductionDetails)
//   Twin of the other production screens, but each machine carries TPI + Spindle
//   and the Target Prodn uses the Simplex formula:
//     round((7.2 * DSpeed) / (TPI * StdHank) / 1000 * Spindle / 8 * (WorkingMins/60), 2)
//   Parent + child transaction (a shift's production for every simplex machine,
//   plus a stoppage sub-grid). Company + FY scoped (from the JWT).
//
//   Header  : sp_Prodn_SimplexProdn_AddEdit       -> SPXProdnCode (ExecuteScalar)
//   Details : sp_Prodn_SimplexProdnDetails_Delete + loop _Insert
//   Stoppage: sp_Prodn_SimplexStoppage_Delete     + loop _Insert
//   Employee: sp_Prodn_SimplexEmployee_Delete(@ShiftCode,@CompanyCode) + loop _Insert
//   No      : sp_Prodn_SimplexProdnNo  @CompanyCode,@FYCode
//   List    : vw_Prodn_SimplexProdn   (CompanyCode + FYCode)
//   Load    : sp_Prodn_SimplexProdnDetails_GetAll @CompanyCode,@SPXProdnCode,@ShiftCode
//             + vw_Prodn_SimplexStoppage
//   Delete  : sp_Prodn_SimplexProdn_Delete @SPXProdnCode
...
// GET /simplex-production/options
export const getOptions = async (req, res) => {
  try {
    ...
    console.error("DB Error (simplexProduction getOptions):", err);
    return sendError(res, err);
  }
};

// GET /simplex-production/machines?branchCode=
export const getMachines = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query(`Select * from vw_Prodn_Simplex_MachineSetting Where ${where} Order by SPXMachineSettingCode`);

    const rows = result.recordset.map((m) => ({
      SPXMachineSettingCode: m.SPXMachineSettingCode,
      ...
      TPI: toNum(m.TPI),
      Spindles: toNum(m.Spindles),
      TargetProdn: toNum(m.TargetProdn),
    }));
    return sendSuccess(res, rows);
  } catch (err) {
    console.error("DB Error (simplexProduction getMachines):", err);
    return sendError(res, err);
  }
};

// GET /simplex-production/next-no
export const getNextNo = async (req, res) => {
  try {
    ...
    const no = await scalar(
      ...
      "sp_Prodn_SimplexProdnNo"
    );
    return sendSuccess(res, { no: toInt(no) });
  } catch (err) {
    console.error("DB Error (simplexProduction getNextNo):", err);
    return sendError(res, err);
  }
};

// GET /simplex-production/exists?date=&shiftCode=&branchCode=
export const checkExisting = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .input("SPXProdnDate", sql.DateTime, D(req.query.date) || new Date())
      .input("CompanyCode", sql.Int, toInt(req.headers.companyCode))
      .input("BranchCode", sql.Int, toInt(req.query.branchCode))
      .query(
        "SELECT SPXProdnCode FROM tbl_Prodn_SimplexProdn WHERE ShiftCode = @ShiftCode " +
          "AND SPXProdnDate = @SPXProdnDate AND CompanyCode = @CompanyCode AND BranchCode = @BranchCode"
      );
    return sendSuccess(res, { exists: result.recordset.length > 0 });
  } catch (err) {
    console.error("DB Error (simplexProduction checkExisting):", err);
    return sendError(res, err);
  }
};

// GET /simplex-production/lists
export const getList = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query(`select * from vw_Prodn_SimplexProdn Where CompanyCode = ${companyCode} AND FYCode = ${fyCode} ORDER BY SPXProdnDate DESC, ShiftNo DESC`);
    const data = result.recordset.map((row) => ({ ...row, id: row.SPXProdnCode }));
    return sendPaginated(res, data, req.query);
  } catch (err) {
    console.error("DB Error (simplexProduction getList):", err);
    return sendError(res, err);
  }
};

// GET /simplex-production/list/:code?shiftCode=
export const getById = async (req, res) => {
  try {
    ...
    const companyCode = toInt(req.headers.companyCode);
    if (!code) return sendError(res, "Invalid SPXProdnCode", 400);
    const pool = await getPool(req.headers.subdbname);

    const det = await pool
      .request()
      .input("CompanyCode", sql.Int, companyCode)
      .input("SPXProdnCode", sql.Int, code)
      .input("ShiftCode", sql.Int, shiftCode)
      .execute("sp_Prodn_SimplexProdnDetails_GetAll");

    if (!det.recordset.length) return sendError(res, "Simplex Production not found", 404);
    const h = det.recordset[0];

    const stop = await pool.request().query(`select * from vw_Prodn_SimplexStoppage where SPXProdnCode = ${code}`);

    return sendSuccess(res, {
      header: {
        SPXProdnCode: h.SPXProdnCode,
        SPXProdnNo: h.SPXProdnNo,
        SPXProdnDate: h.SPXProdnDate,
      ...
      details: det.recordset.map((d) => ({
        SPXMachineSettingCode: d.SPXMachineSettingCode,
    ...
    console.error("DB Error (simplexProduction getById):", err);
    return sendError(res, err);
  }
};
...
function computeDetails(rawRows, stoppages, workingMins, totalWasteKgs) {
  ...
  const rows = rawRows.map((d) => {
    ...
    const spindle = toNum(d.Spindles);
    const energy = toNum(d.Energy);
    const st = stopByMc.get(toInt(d.MachineCode)) || { mins: 0, reasons: [] };
    const stoppage = st.mins;
    // Simplex: TargetProdn = round((7.2*DSpeed)/(TPI*StdHank)/1000*Spindle/8*(WorkingMins/60), 2)
    const targetProdn = tpi > 0 && stdHank > 0
      ? r2((7.2 * dSpeed) / (tpi * stdHank) / 1000 * spindle / 8 * (workingMins / 60))
      : 0;
    ...
    return {
      ...d, prodn, dSpeed, stdHank, tpi, spindle, energy, stoppage, targetProdn, diff, util, effi, index, ukg,
...
const saveOrUpdate = async (req, res, isEdit) => {
  ...
  try {
    ...
    const code = isEdit ? toInt(req.params.code ?? b.SPXProdnCode) : null;
    if (isEdit && !code) return sendError(res, "Invalid SPXProdnCode for update", 400);
    ...
    if (!isEdit) {
      const dup = await pool
        .request()
        .input("ShiftCode", sql.Int, shiftCode)
        .input("SPXProdnDate", sql.DateTime, D(b.SPXProdnDate) || new Date())
        .input("CompanyCode", sql.Int, companyCode)
        .input("BranchCode", sql.Int, branchCode)
        .query(
          "SELECT SPXProdnCode FROM tbl_Prodn_SimplexProdn WHERE ShiftCode=@ShiftCode AND SPXProdnDate=@SPXProdnDate AND CompanyCode=@CompanyCode AND BranchCode=@BranchCode"
        );
    ...
    const spxProdnNo = isEdit
      ? toInt(b.SPXProdnNo)
      : toInt(await scalar(pool.request().input("CompanyCode", sql.Int, companyCode).input("FYCode", sql.Int, fyCode), "sp_Prodn_SimplexProdnNo"));

    tx = new sql.Transaction(pool);
    await tx.begin();

    const head = new sql.Request(tx);
    if (isEdit && code) head.input("SPXProdnCode", sql.Int, code);
    head.input("SPXProdnNo", sql.Int, spxProdnNo);
    head.input("SPXProdnDate", sql.DateTime, D(b.SPXProdnDate) || new Date());
    ...
    head.input("TotalTPI", sql.Decimal(18, 3), totals.tpi);
    head.input("TotalSpindles", sql.Decimal(18, 3), totals.spindles);
    ...
    const spxProdnCode = toInt(await scalar(head, "sp_Prodn_SimplexProdn_AddEdit"));

    await new sql.Request(tx).input("SPXProdnCode", sql.Int, spxProdnCode).execute("sp_Prodn_SimplexProdnDetails_Delete");

    let sno = 0;
    for (const r of rows) {
      sno += 1;
      await new sql.Request(tx)
        .input("SPXProdnCode", sql.Int, spxProdnCode)
        ...
        .input("TPI", sql.Decimal(18, 3), r.tpi)
        .input("Spindles", sql.Decimal(18, 3), r.spindle)
        .input("Indexs", sql.Decimal(18, 3), r.index)
        .input("CountNameCode", sql.Int, toInt(r.CountNameCode))
        .input("SPXMachineSettingCode", sql.Int, toInt(r.SPXMachineSettingCode))
        ...
        .execute("sp_Prodn_SimplexProdnDetails_Insert");
    }

    await new sql.Request(tx).input("SPXProdnCode", sql.Int, spxProdnCode).execute("sp_Prodn_SimplexStoppage_Delete");
    for (const s of stoppages) {
      await new sql.Request(tx)
        .input("SPXProdnCode", sql.Int, spxProdnCode)
        ...
        .execute("sp_Prodn_SimplexStoppage_Insert");
    }

    await new sql.Request(tx)
      .input("ShiftCode", sql.Int, shiftCode)
      .input("CompanyCode", sql.Int, companyCode)
      .execute("sp_Prodn_SimplexEmployee_Delete");
    for (const r of rows) {
      await new sql.Request(tx)
        .input("ShiftCode", sql.Int, shiftCode)
        .input("CompanyCode", sql.Int, companyCode)
        .input("MachineCode", sql.Int, toInt(r.MachineCode))
        .input("EmployeeCode", sql.Int, toInt(r.EmployeeCode))
        .execute("sp_Prodn_SimplexEmployee_Insert");
    }

    await tx.commit();
    return sendSuccess(
      res,
      { SPXProdnCode: spxProdnCode },
    ...
    console.error("DB Error (saveOrUpdate simplexProduction):", err);
    return sendError(res, err);
  }
};
...
// DELETE /simplex-production/delete/:code
export const remove = async (req, res) => {
  try {
    ...
    if (!code) return sendError(res, "Invalid SPXProdnCode", 400);
    const pool = await getPool(req.headers.subdbname);
    await pool.request().input("SPXProdnCode", sql.Int, code).execute("sp_Prodn_SimplexProdn_Delete");
    return sendSuccess(res, null, "The record is deleted");
  } catch (err) {
    if (err.message && (err.message.includes("REFERENCE") || err.message.includes("FK_"))) {
      return sendError(res, "You can not delete this Simplex Production!", 409);
    }
    console.error("DB Error (simplexProduction remove):", err);
    return sendError(res, err);
  }
};
