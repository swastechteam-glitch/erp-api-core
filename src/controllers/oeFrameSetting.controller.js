...
// ---------------------------------------------------------------------------
// OE Frame Setting master
//   (port of WinForms frmOEFrameSetting / frmOEFrameSettingDetails)
//   OE-table twin of Spinning Frame Setting: Department + Machine + Count +
//   Mixing dropdowns + GPS / Actual Count / 40s Conversion Value / TPI / Speed /
//   Effi / Utilization / Actual Rotter (Spindle) / Rotter (Spindle) Constant +
//   Std Prodn (computed) + Status.
//
//   Selecting a Count auto-fills Actual Count / 40s Value / Speed / TPI /
//   Std Hank / Effi / Utilization from the OE Count Setting.
//
//   Std Prodn = round((7.2 * Speed * Effi) / (ActualCount * TPI * 100) * 1.5, 2)
//               (OE variant — differs from Spinning: no Utilization/Spindle, * 1.5.)
//
//   - List   : EXEC sp_Prodn_OE_FrameSetting_GetAll  @CompanyCode
//   - Create : EXEC sp_Prodn_OE_FrameSetting_AddEdit (without @spgFrameSettingCode)
//   - Update : EXEC sp_Prodn_OE_FrameSetting_AddEdit (with @spgFrameSettingCode)
//   - Delete : EXEC sp_Prodn_OE_FrameSetting_Delete   (@spgFrameSettingCode)
//
// Department auto-locks to the existing OE Count Setting department; the Machine
// dropdown shows that department's un-allotted machines (sp_Prodn_OE_FrameSetting_BindData),
// with the edited record's own machine kept available. Company-scoped (JWT).
...
// VB GetActualHankProduction (OE variant) — Std Prodn (Target).
const calcStdProdn = (speed, effi, actualCount, tpi) =>
  actualCount > 0 && tpi > 0
    ? r2((7.2 * speed * effi) / (actualCount * tpi * 100) * 1.5)
    : 0;

// GET /oe-frame-setting/options  -> dropdown lookups
export const getOptions = async (req, res) => {
  try {
    ...
    // Count dropdown carries the OE Count Setting values so the React form can
    ...
    const countSql =
      ...
      "Speed, TPI, STDHank, Effi, Utilization from vw_Prodn_OE_CountSetting Order By CountName";
    ...
    // Machines: department's un-allotted machines (VB sp_Prodn_OE_FrameSetting_BindData).
    let machines = [];
    if (lockedDept) {
      const mc = await pool
        .request()
        .input("CompanyCode", sql.Int, companyCode)
        .input("DepartmentCode", sql.Int, lockedDept)
        .execute("sp_Prodn_OE_FrameSetting_BindData");
      machines = mc.recordset.map((r) => ({ value: r.MachineCode, label: r.MachineName }));

      // On edit, keep the record's own machine available (it is already allotted,
      // so BindData would exclude it).
      if (editCode && !machines.some((m) => String(m.value) === String(0))) {
        const cur = await pool
          .request()
          .input("SpgFrameSettingCode", sql.Int, editCode)
          .input("CompanyCode", sql.Int, companyCode)
          .query(
            "SELECT TOP 1 MachineCode, MachineName FROM vw_Prodn_OE_FrameSetting " +
              "WHERE SpgFrameSettingCode = @SpgFrameSettingCode AND CompanyCode = @CompanyCode"
          );
        const row = cur.recordset[0];
        if (row && !machines.some((m) => String(m.value) === String(row.MachineCode))) {
          machines.unshift({ value: row.MachineCode, label: row.MachineName });
        }
      }
    }
    ...
    console.error("DB Error (getOptions oe-frame-setting):", err);
    return sendError(res, err);
  }
};

// GET /oe-frame-setting/lists
export const getList = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .input("CompanyCode", sql.Int, companyCode)
      .execute("sp_Prodn_OE_FrameSetting_GetAll");
    ...
    console.error("DB Error (getList oe-frame-setting):", err);
    return sendError(res, err);
  }
};

// GET /oe-frame-setting/list/:spgFrameSettingCode
export const getById = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .input("CompanyCode", sql.Int, companyCode)
      .query(
        "SELECT * FROM vw_Prodn_OE_FrameSetting " +
          "WHERE SpgFrameSettingCode = @SpgFrameSettingCode AND CompanyCode = @CompanyCode"
      );

    if (!result.recordset.length) return sendError(res, "OE Frame Setting not found", 404);

    const row = result.recordset[0];
    return sendSuccess(res, { ...row, StatusText: STATUS_LABEL(row.Status) });
  } catch (err) {
    console.error("DB Error (getById oe-frame-setting):", err);
    return sendError(res, err);
  }
};

// Shared add/edit handler -> EXEC sp_Prodn_OE_FrameSetting_AddEdit
const saveOrUpdate = async (req, res, isEdit) => {
  try {
    ...
    const stdProdn = calcStdProdn(speed, effi, actualCount, tpi);
    ...
    const code = isEdit ? parseInt(req.params.spgFrameSettingCode ?? body.SpgFrameSettingCode) : null;
    if (isEdit && !code) return sendError(res, "Invalid SpgFrameSettingCode for update", 400);
    ...
    await request.execute("sp_Prodn_OE_FrameSetting_AddEdit");
    ...
    console.error("DB Error (saveOrUpdate oe-frame-setting):", err);
    return sendError(res, err);
  }
};

// POST /oe-frame-setting/create
export const create = (req, res) => saveOrUpdate(req, res, false);
// PUT  /oe-frame-setting/update/:spgFrameSettingCode
export const update = (req, res) => saveOrUpdate(req, res, true);

// DELETE /oe-frame-setting/delete/:spgFrameSettingCode
export const remove = async (req, res) => {
  try {
    ...
    await pool
      .request()
      .input("spgFrameSettingCode", sql.Int, code)
      .execute("sp_Prodn_OE_FrameSetting_Delete");

    return sendSuccess(res, null, "The record is deleted");
  } catch (err) {
    if (err.message && (err.message.includes("REFERENCE") || err.message.includes("FK_"))) {
      return sendError(res, "You can not delete the OE Frame Setting!", 409);
    }
    console.error("DB Error (remove oe-frame-setting):", err);
    return sendError(res, err);
  }
};
